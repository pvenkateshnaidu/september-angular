...
let BenchSalesService = class BenchSalesService {
    constructor(http) {
        this.http = http;
    }
    getConsultants() {
...
let BenchSalesModule = class BenchSalesModule {
};
...
let UserCreateComponent = class UserCreateComponent {
    constructor(route, messageService, restService, router) {
        this.route = route;
    ...
    ngOnInit() {
        this.restService.getConsultantsList().subscribe((response) => {
    ...
    onChange(event) {
        ...
        this.restService.editConsultant(event.value).subscribe((response) => {
    ...
    ChangeVendor(event) {
        ...
        this.vcname = '';
        this.contacts = [];
        this.restService.editVenodr(event.value).subscribe((response) => {
            this.contacts = response.contacts;
        }, (error) => console.log(error));
    }
    ChangeContactsInner(value) {
        ...
        this.restService.getContactDetails(formData).subscribe((response) => {
    ...
    registerClientForm() {
        this.restService.storeClient(this.registerClient).subscribe(response => {
    ...
    registerContactForm() {
        this.restService.storeContact(this.registerContact).subscribe(response => {
            ...
            this.restService.editVenodr(this.registerForm.value.vid).subscribe((response2) => {
    ...
    registerVendorCompany() {
        ...
        this.restService.storeVendor(this.registerVendor).subscribe(response => {
            console.log(response),
            ...
            this.restService.getConsultantsList().subscribe((response1) => {
                // console.log(this.states = response.submissions);
                console.log(this.vendors = response1.vendorslist);
                // console.log( this.clients =  response1.clients);
            }, (error) => { console.log(error); });
            this.restService.editVenodr(response.vendorId).subscribe((response2) => {
                this.contacts = response2.contacts;
                this.selectedContacts = response.contactId;
                this.ChangeContactsInner(response.contactId);
            }, (error) => console.log(error));
    ...
    registerUser() {
        ...
        this.restService.storeUser(this.registerForm).subscribe(response => {
            console.log(response),
...
let UserEditComponent = class UserEditComponent {
    ...
    ngOnInit() {
        ...
        this.restService.editUser(id).subscribe((response) => {
    ...
    imagesPreviewResume(event) {
        if (event.target.files && event.target.files[0]) {
            ...
            this.restService.storeDocument(formData).subscribe((response) => {
                console.log(response);
                this.registerForm.patchValue({
    ...
    updateUserDetails() {
        let id = this.route.snapshot.params.id;
        this.restService.updateUser(this.registerForm, id).subscribe((response) => {
            console.log(response),
                this.router.navigate(['jobs/list']);
        }, error => {
            this.serverErrors = error.error.errors;
...
//# sourceMappingURL=admin-benchsales-benchsales-module-es2015.js.map